...
path <- file.path("../readMapping/mappedReads_fromprof", targets$sample, "abundance.tsv") # set file paths to your mapped data, added '../readMapping' to point to the correct upper subdir
...
Tx <- transcripts(EnsDb.Hsapiens.v86, columns=c("tx_id", "gene_name")) # only pulling the two columns needed
Tx <- as_tibble(Tx)
...
Txi_gene <- tximport(path, 
                     type = "kallisto", 
                     tx2gene = Tx, 
                     txOut = FALSE, #How does the result change if this =FALSE vs =TRUE? # FALSE = will output gene level data, other wise transcript level data
...
path <- file.path("../readMapping/mappedReads_fromprof", targets$sample, "abundance.tsv") # set file paths to your mapped data
