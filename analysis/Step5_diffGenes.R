...
design <- model.matrix(~0 + group) # ~0: no intercept in the model, so that each group gets its own coefficient. If you had an intercept, then one of the groups would be the reference group and would not get its own coefficient
...
v.DEGList.filtered.norm <- voom(myDGEList.filtered.norm, design, plot = TRUE) # voom: variance modeling at the observational level. This function transforms the count data to log2-counts per million (logCPM) and estimates the mean-variance relationship, which is important for linear modeling. The 'plot=TRUE' argument will produce a plot of the mean-variance trend, which can help you assess whether the voom transformation has successfully stabilized the variance across different expression levels.
# description of plots: lowly expressed genes have slightly higher variance, weights: precison weights applied to each gene 
# fit a linear model to your data
fit <- lmFit(v.DEGList.filtered.norm, design)

# Contrast matrix ----
contrast.matrix <- makeContrasts(infection = disease - healthy, # disease first: want genes to be positive if they are higher in the disease state, and neg if lower in disease state
                                 levels=design)
...
ebFit <- eBayes(fits)
#write.fit(ebFit, file="lmfit_results.txt") # to write out the statistics 

# TopTable to view DEGs -----
#myTopHits <- topTable(ebFit, adjust ="BH", coef=1, number=10, sort.by="logFC") # look at top differentially expressed genes, coef=1 (top based on disease)

# to get top up-regulated genes only (my code)
...
vplot <- ggplot(myTopHits.df) +
  ...
  theme_bw()
# understanding the plot: to the right (positive logFC) are genes that are higher in the disease state (up regulated in disease), and to the left (negative logFC) are genes that are lower in the disease state. The higher up on the plot, the more significant the gene is. The dashed lines indicate thresholds for significance (horizontal line) and fold change (vertical lines). The shaded areas indicate genes that meet both thresholds for significance and fold change.
# magnitude of change (x) vs significance (y)
# the higher up the more significant the gene (in the vocano plot in the pos or neg direction)

# Now make the volcano plot above interactive with plotly
ggplotly(vplot)

# decideTests to pull out the DEGs and make Venn Diagram ----
# decideTests will look at bayseian stats and asks if each of the genes meet the specified cut offs (p.value, lfc - log fold change); -1: down in disease, 0: not significant (not differentially expressed in either condition: disease vs healthy), 1: up in disease
...
head(v.DEGList.filtered.norm$E) # where the expression data resides, log2 counts
colnames(v.DEGList.filtered.norm$E) <- sampleLabels

diffGenes <- v.DEGList.filtered.norm$E[results[,1] !=0,] # only want the rows where the log2 counts is not equal to 0: i want any row this is differentially expressed (1 or -1)
head(diffGenes)
dim(diffGenes)
