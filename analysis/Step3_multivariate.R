...
# need to transpose the data so that samples are rows and genes are columns (reason for t(log2.cpm.filtered.norm))
# show dissimilarity or similarity the samples are to each other
...
# decides how samples should be clustered based on the distance matrix
...
plot(clusters, labels=sampleLabels) # can be used to identify outliers based on tree structure
...
# meaning proportion of variable (e.g. 55.6% of variance accounted for by PC1) --> can get the bulk of the variance of the data through PC1 and PC2
# PC1 and PC2 examples the bulk of the variance of the data
# there are 10 principal components because there are 12 samples
summary(pca.res) # Prints variance summary for all principal components.

pca.res$rotation #$rotation shows you how much each gene influenced each PC (called 'scores')
pca.res$x # 'x' shows you how much each sample influenced each PC (called 'loadings'); can see samples that are at one end of a PC (neg val) and the other end of a PC (pos val)

#note that these have a magnitude and a direction (this is the basis for making a PCA plot)
screeplot(pca.res) # A screeplot is a standard way to view eigenvalues for each PCA (shows the proportion of variance)
...
pc.per # captured values from summary(pca.res)

# Visualize your PCA result ------------------
#lets first plot any two PCs against each other
#We know how much each sample contributes to each PC (loadings), so let's plot
# PC1 is explaining group/disease status (disease vs healthy -> can tell based on grouping by color)
# PC2 looks like it's telling about outlier (one point is much lower than the others) --> specifically HS05
pca.res.df <- as_tibble(pca.res$x)
ggplot(pca.res.df) +
  aes(x=PC1, y=PC2, label=sampleLabels, color=group) +
  geom_point(size=4) +
  # geom_label() +
  stat_ellipse() + # plotting data ellipses around samples
...
# getting PC3 and PC4 (in addition to PC1 and PC2)
