...
all.samples.mouse <- h5read(archs4.mouse, name="meta/samples/geo_accession") # getting accession numbers of the mice
...
my.sample.locations <- which(all.samples.mouse %in% mySamples) # first time you've seen the %in% operator; each record says the location of where the samples are in the all.samples.mouse array
# extract gene symbols from the metadata
genes <- h5read(archs4.mouse, "meta/genes/symbol") # getting more information from the metagroup; each element is a gene symbol

# Extract expression data from ARCHS4 ----
expression <- h5read(archs4.mouse, "data/expression",
                     index=list(my.sample.locations, 1:length(genes))) # getting expression data for just our samples
# transpose to get genes as rows and samples as columns
expression <- t(expression)

rownames(expression) <- genes # rows: gene symbols
colnames(expression) <- all.samples.mouse[my.sample.locations] # columns: sample names
colSums(expression) #this shows the sequencing depth for each of the samples you've extracted; numbers show counts 
archs4.dgelist <- DGEList(expression) # count matrix
archs4.cpm <- cpm(archs4.dgelist) # getting counts per million
colSums(archs4.cpm)

# Filter and normalize the extracted data ----
table(rowSums(archs4.dgelist$counts==0)==12) # 18275 --> genes with 0 counts in all 12 samples
keepers <- rowSums(archs4.cpm>1)>=2 # greater than 1 count per million in greater than equal to 2 samples
archs4.dgelist.filtered <- archs4.dgelist[keepers,]
dim(archs4.dgelist.filtered)
archs4.dgelist.filtered.norm <- calcNormFactors(archs4.dgelist.filtered, method = "TMM") # normalizing the data; now has normalization factors
...
sample_title <- h5read(archs4.mouse, name="meta/samples/title") # sample name
# extract sample characteristics
sample_characteristics<- h5read(archs4.mouse, name="meta/samples/characteristics_ch1") # more information about each sample
...
pc.per # most variance can be accounted for in just PC1 alone
...
ggplot(pca.res.df) +
  aes(x=PC1, y=PC2, color = treatment, shape = genotype) + # LPS (treatment) drives a huge amount of variation; PC2 really separates the genotype
  geom_point(size=4) +
  # geom_label() +
  # stat_ellipse() +
...
ggplot(pca.pivot) +
  ...
  theme_bw() +
  coord_flip()
# results: LPS treatment is really driving the principal component (directional difference of LPS and treatment)
# PC2: really separtes ripk3casp8 compared to the other two (WT and ripk3) --> shows clear separation between the genotypes 
